...
source_folder = 'D:/16/HW_WCS_15'  # Overlapped images
destination_folder = 'D:/16/15_denoised'  # Output folder
0.
...
def undo_vertical_overlap(image):
    ...
    shifted_img = np.roll(img_f, shift=6, axis=0)
    ...
    recovered = np.clip(recovered, 20, 255).astype(np.uint8)
    _, img = cv2.threshold(recovered, 90, 255, cv2.THRESH_BINARY)
    #_, binary = cv2.threshold(recovered, 127, 255, cv2.THRESH_BINARY_INV)
...
print("Vertical overlap removal complete.")
